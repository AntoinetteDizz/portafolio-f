...
export default async function Home() {
  ...
  const testBackendConnection = async () => {
    try {
      const [healthResponse, saludoResponse] = await Promise.all([
        fetchWithTimeout(`${apiUrl}/api`, { 
          cache: 'no-store',
          next: { revalidate: 0 }
        }),
        fetchWithTimeout(`${apiUrl}/api/saludo`, {
          cache: 'no-store',
          next: { revalidate: 0 }
        })
      ]);
      ...
      if (!saludoResponse.ok) throw new Error(`Saludo endpoint failed: ${saludoResponse.status}`);

      return {
        apiStatus: await healthResponse.json() as ApiStatus,
        saludoData: await saludoResponse.json() as SaludoData
      };
    } catch (error) {
  ...
  try {
    const { apiStatus: healthData, saludoData: saludo } = await testBackendConnection();
    apiStatus = { ...healthData, timestamp: new Date().toISOString() };
    saludoData = { ...saludo, timestamp: new Date().toISOString() };
  } catch (error) {
    apiStatus = { 
      status: '❌ Error de conexión con el backend',
      timestamp: new Date().toISOString()
    };
    saludoData = { 
        ...
        <div className="w-full max-w-md p-6 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg border border-blue-100 dark:border-gray-700 transition-all hover:shadow-xl">
          ...
          <div className="px-4 py-3 bg-white/70 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
            <p className={`text-lg font-mono text-center ${
              saludoData.message.includes('Error') ? 'text-red-500 dark:text-red-400' : 
